...
// Initialize Google Sheets API (for payment history queries)
let auth;
...
import { updateOrderPayment, updateOrderPaymentWithEvidence } from './google-sheets.js';
import { 
  getJakartaTodayISO, 
  addDaysJakarta, 
  toISODateJakarta 
...
async function extractAmountFromImage(message, options = {}) {
  try {
    ...
    console.log('🔍 [OCR_AMOUNT] Using new OCR service with watermark-resistant preprocessing...');
    ...
    if (result.ok) {
      ...
      if (result.needsConfirmation) {
        console.log(`⚠️ [OCR_AMOUNT] Low confidence or multiple candidates - may need user confirmation`);
      ...
      console.log(`❌ [OCR_AMOUNT] Extraction failed: ${result.reason}`);
      if (result.error) {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(`🔍 [OCR_AMOUNT] Using 'eng+ind' language for better Indonesian number recognition`);
      } catch (langError) {
        console.log(`⚠️ [OCR_AMOUNT] 'eng+ind' not available, using 'eng' only`);
        worker = await createWorker('eng');
      }
      
      try {
        console.log(`🔍 [OCR_AMOUNT] Processing image for amount extraction...`);
        ...
        for (const psm of psmModes) {
          try {
            console.log(`🔍 [OCR_AMOUNT] Trying PSM mode ${psm}...`);
            ...
            console.log(`  PSM ${psm}: Found "Rp"=${hasRp}, Found amount pattern=${hasAmountPattern}, "Rp" matches=${rpMatches.length}, Text length=${text.length}`);
            ...
            if (hasRp) {
              ...
              bestPsm = psm;
              bestHasRp = true;
              console.log(`✅ [OCR_AMOUNT] PSM ${psm} found "Rp" - using this result`);
        ...
        console.log(`🔍 [OCR_AMOUNT] Full extracted text length: ${text.length} chars`);
        
        // Show full text if it's not too long (for debugging)
        if (text.length < 2000) {
          console.log(`🔍 [OCR_AMOUNT] Full extracted text:\n"${text}"`);
        }
        ...
        if (linesWithRp.length > 0) {
          console.log(`🔍 [OCR_AMOUNT] Lines containing "Rp":`);
        ...
        if (rpMatches.length > 0) {
          ...
          console.log(`⚠️ [OCR_AMOUNT] No amounts with "Rp" prefix found in image`);
          console.log(`⚠️ [OCR_AMOUNT] OCR might have misread "Rp" or amount format is different`);
          console.log(`🔍 [OCR_AMOUNT] Will try fallback patterns to find amounts without "Rp" prefix`);
        }
        ...
        if (extractedAmounts.length === 0) {
          console.log(`🔍 [OCR_AMOUNT] Trying fallback patterns for amounts without "Rp" prefix...`);
        ...
        if (uniqueAmounts.length > 0) {
          ...
          console.log(`⚠️ [OCR_AMOUNT] No candidate amounts found in fallback patterns`);
        ...
        if (extractedAmount) {
          ...
          console.log(`ℹ️ [OCR_AMOUNT] No valid amount pattern found in extracted text`);
          console.log(`ℹ️ [OCR_AMOUNT] Will fall back to expected amount from order`);
        }
      } finally {
        await worker.terminate();
      }
      ...
      console.log(`ℹ️ [OCR_AMOUNT] Error: ${tesseractError.message}`);
    }
...
async function extractOrderIdFromImage(message, sendMessage) {
  try {
    ...
    try {
      ...
      try {
        console.log(`🔍 [OCR] Processing image for order ID extraction...`);
        const { data: { text } } = await worker.recognize(imageUrl);
        ...
        if (match) {
          ...
          console.log(`✅ [OCR] Found order ID in image: ${extractedId}`);
          await worker.terminate();
          return extractedId;
        } else {
          console.log(`ℹ️ [OCR] Order ID pattern not found in extracted text`);
        }
      } finally {
        await worker.terminate();
      }
      ...
      console.log(`ℹ️ [OCR] Error: ${tesseractError.message}`);
    }
...
// Pending payment confirmations (key: userId:orderId, value: { orderId, expectedAmount, enteredAmount, timestamp })
const pendingPaymentConfirmations = new Map();
...
setInterval(() => {
  ...
  if (cleaned > 0) {
    console.log(`🧹 [PAY_CONFIRM] Cleaned up ${cleaned} expired payment confirmations`);
  }
...
export async function handlePayWithEvidence(chatId, userId, message, sendMessage) {
  console.log(`📸 [PAY_EVIDENCE] Processing payment evidence - chatId: ${chatId}, userId: ${userId}`);
  ...
  try {
    ...
    console.log(`🔍 [PAY_EVIDENCE] Caption: "${caption}", Message text: "${messageText}", Combined: "${combinedText}"`);
    ...
    if (!orderIdMatch && (message.photo || message.document)) {
      console.log(`🔍 [PAY_EVIDENCE] Order ID not found in text, attempting OCR from image...`);
      try {
        const extractedOrderId = await extractOrderIdFromImage(message, sendMessage);
        if (extractedOrderId) {
          orderId = extractedOrderId;
          console.log(`✅ [PAY_EVIDENCE] Extracted order_id from image via OCR: ${orderId}`);
        }
      } catch (ocrError) {
    ...
    console.log(`🔍 [PAY_EVIDENCE] Extracted order_id: ${orderId}`);
    ...
    let extractedAmount = null;
    if (evidenceFileId) {
      console.log(`🔍 [PAY_EVIDENCE] Attempting to extract amount from image...`);
      console.log(`🔍 [PAY_EVIDENCE] Expected amount from order: Rp ${expectedAmount.toLocaleString('id-ID')}`);
      console.log(`🔍 [PAY_EVIDENCE] Evidence file_id: ${evidenceFileId}, type: ${evidenceType}`);
      try {
        ...
        if (extractedAmount) {
          ...
          console.log(`⚠️ [PAY_EVIDENCE] Could not extract amount from image - OCR returned null`);
          console.log(`ℹ️ [PAY_EVIDENCE] Possible reasons:`);
          console.log(`   1. tesseract.js not installed (run: npm install tesseract.js)`);
          console.log(`   2. No "Rp" prefix found in image`);
          console.log(`   3. Amount not clearly visible in image`);
      ...
      console.log(`⚠️ [PAY_EVIDENCE] No evidence file_id found, cannot extract amount from image`);
    }
    ...
    if (extractedAmount && extractedAmount > 0) {
      ...
      console.log(`🔍 [PAY_EVIDENCE] Will use extracted amount instead of expected amount`);
      // Check for suspicious amount if we extracted from image
      const { detectSuspiciousPayment, parseIDRAmount } = await import('./payment-tracker.js');
      ...
      if (suspiciousCheck.isSuspicious) {
        ...
        console.log(`⚠️ [PAY_EVIDENCE] Suspicious amount detected: ${suspiciousReason}`);
      } else {
      ...
      console.log(`⚠️ [PAY_EVIDENCE] No extracted amount available, using expected amount`);
    ...
    // PAYMENT HISTORY: Create payment record
    const { appendPaymentHistory, recalcOrderPaymentSummary } = await import('./google-sheets.js');
    const { formatPrice } = await import('./price-calculator.js');
    
    const paymentRecord = {
      order_id: orderId,
      amount_input: paymentAmount,
      amount_confirmed: needsVerification ? null : paymentAmount, // Only confirm if not suspicious
      payment_method: 'transfer',
      status: needsVerification ? 'pending_verification' : 'confirmed',
      note: needsVerification ? `Perlu verifikasi: ${suspiciousReason}` : `Pembayaran via transfer - ${amountSource}`,
      proof_file_id: evidenceFileId || '',
      proof_caption: (message.caption || message.text || '').trim(),
      created_by: `${userId}`, // Telegram user_id
      currency: 'IDR',
    };
    
    const payment_id = await appendPaymentHistory(paymentRecord);
    console.log(`✅ [PAY_EVIDENCE] Payment record created: ${payment_id}, status: ${paymentRecord.status}`);
    
    // If needs verification, ask for confirmation
    if (needsVerification) {
      // Store pending confirmation with payment_id for later update
      const confirmationKey = `${userId}:${orderId}`;
      pendingPaymentConfirmations.set(confirmationKey, {
        payment_id, // Store payment_id for later update
        orderId,
        expectedAmount,
      ...
      // Store pending payment_id for simple "Ya"/"Tidak" confirmation
      const { setPendingPaymentConfirmation } = await import('./server.js');
      if (setPendingPaymentConfirmation) {
        setPendingPaymentConfirmation(chatId, payment_id);
      }
      ...
      await sendMessage(
        ...
        `💳 Jumlah yang Terdeteksi: Rp ${formatPrice(paymentAmount)}\n\n` +
        `**Peringatan:** ${suspiciousReason}\n\n` +
        'Apakah jumlah ini benar?\n\n' +
        '👉 Balas: **Ya** untuk konfirmasi\n' +
        '👉 Balas: **Tidak** untuk tolak'
      );
      return;
    }
    
    // If not suspicious, recalculate order payment summary
    const summary = await recalcOrderPaymentSummary(orderId);
    console.log(`✅ [PAY_EVIDENCE] Payment confirmed & order summary updated - Order: ${orderId}, Total Paid: ${summary.total_paid}, Status: ${summary.payment_status}`);
    ...
    confirmationMessage += 
      `💳 Total Sudah Dibayar: Rp ${formatPrice(summary.total_paid)}\n` +
      `📊 Sisa: Rp ${formatPrice(summary.remaining_amount)}\n` +
...
export async function handlePay(chatId, userId, orderId, amountInput, sendMessage) {
  ...
  try {
    ...
    const { parseIDRAmount, detectSuspiciousPayment } = await import('./payment-tracker.js');
    ...
    if (expectedAmount > 0) {
      ...
      if (suspicious.isSuspicious) {
        ...
        console.log(`⚠️ [PAY] Suspicious amount detected: ${suspiciousReason}`);
      }
    }
    ...
    const paymentRecord = {
      order_id: orderId,
      amount_input: newPaymentAmount,
      amount_confirmed: needsVerification ? null : newPaymentAmount,
      payment_method: 'manual', // Admin manual entry
      status: needsVerification ? 'pending_verification' : 'confirmed',
      note: needsVerification ? `Perlu verifikasi: ${suspiciousReason}` : 'Pembayaran manual entry by admin',
      proof_file_id: '',
      proof_caption: amountInput, // Store original input
      created_by: `${userId}`, // Telegram user_id
      currency: 'IDR',
    };
    
    const payment_id = await appendPaymentHistory(paymentRecord);
    console.log(`✅ [PAY] Payment record created: ${payment_id}, status: ${paymentRecord.status}`);
    
    // If needs verification, ask for confirmation
    if (needsVerification) {
      // Store pending confirmation with payment_id
      const confirmationKey = `${userId}:${orderId}`;
      pendingPaymentConfirmations.set(confirmationKey, {
        payment_id, // Store payment_id for later update
        orderId,
    ...
    // Update order payment summary from Payment_History
    const summary = await recalcOrderPaymentSummary(orderId);
    
    console.log(`✅ [PAY] Payment confirmed & order summary updated - Order: ${orderId}, Total Paid: ${summary.total_paid}, Status: ${summary.payment_status}`);
    ...
    const message = 
      ...
      `💰 Payment ID: \`${payment_id}\`\n` +
      `💵 Total Pesanan: Rp ${formatPrice(expectedAmount)}\n` +
      `💳 Pembayaran Ini: Rp ${formatPrice(newPaymentAmount)}\n` +
      `💳 Total Sudah Dibayar: Rp ${formatPrice(summary.total_paid)}\n` +
      `📊 Sisa: Rp ${formatPrice(summary.remaining_amount)}\n` +
      `✅ Status: ${summary.payment_status}`;
 ...
 */
/**
 * Handle /orders_unpaid command
 * List orders that are NOT fully paid (unpaid + partial)
 * Format output sama dengan /orders_date
 * @param {number} chatId - Telegram chat ID
 * @param {number} userId - Telegram user ID
 * @param {string} dateStr - Date string (YYYY-MM-DD, DD/MM/YYYY, 'today', 'tomorrow', or empty for today)
 * @param {Function} sendMessage - Function to send Telegram message
 */
export async function handleOrdersUnpaid(chatId, userId, dateStr, sendMessage) {
  try {
    console.log(`🔍 [ORDERS_UNPAID] Command received - userId: ${userId}, dateStr: "${dateStr || 'today (default)'}"`);
    ...
    let targetDate;
    const effectiveDateStr = dateStr || 'today';
    
    if (effectiveDateStr === 'today' || effectiveDateStr === 'hari ini') {
      targetDate = getTodayDate();
      console.log(`[ORDERS_UNPAID] todayISO=${targetDate}`);
    } else if (effectiveDateStr === 'tomorrow' || effectiveDateStr === 'besok') {
      targetDate = getTomorrowDate();
      console.log(`[ORDERS_UNPAID] tomorrowISO=${targetDate}`);
    } else {
      // Validate and normalize date format
      const normalized = toISODateJakarta(effectiveDateStr);
      if (!normalized) {
        await sendMessage(chatId, '❌ Format tanggal tidak valid. Gunakan: YYYY-MM-DD atau DD/MM/YYYY\n\nContoh: /orders_unpaid 2026-01-18\nAtau: /orders_unpaid today');
        return;
      }
      targetDate = normalized;
    }
    
    console.log(`🔍 [ORDERS_UNPAID] Fetching unpaid orders for date: ${targetDate}`);
    
    // Get ALL orders for target date (no payment filter yet)
    const allOrders = await getOrdersByDate(targetDate, null);
    console.log(`✅ [ORDERS_UNPAID] Found ${allOrders.length} total orders for ${targetDate}`);
    
    // Filter: only orders that are NOT fully paid
    const unpaidOrders = allOrders.filter(order => {
      ...
      const totalPaid = parseFloat(order.total_paid || order.paid_amount || 0);
      
      // Include if totalPaid < totalAmount
      // Definition: "belum full paid" = total_paid < total_amount
      if (totalAmount > 0 && totalPaid < totalAmount) {
        console.log(`🔍 [ORDERS_UNPAID] Order ${order.id} included - total: ${totalAmount}, paid: ${totalPaid}`);
        return true;
      }
    ...
    console.log(`✅ [ORDERS_UNPAID] Filtered to ${unpaidOrders.length} unpaid/partial orders`);
    
    // Format and send list message (reuse existing formatter, add payment info)
    const message = formatOrderListMessageWithPayment(unpaidOrders, targetDate);
    await sendMessage(chatId, message);
    
    console.log(`✅ [ORDERS_UNPAID] Order list sent successfully`);
  } catch (error) {
...
/**
 * Format order list message WITH payment info (for /orders_unpaid)
 * Same structure as formatOrderListMessage, but adds payment details
 * @param {Array} orders - Array of orders
 * @param {string} date - Date string
 * @returns {string} Formatted message
 */
function formatOrderListMessageWithPayment(orders, date) {
  if (orders.length === 0) {
    return `✅ Tidak ada order yang belum lunas untuk tanggal ${date}.`;
  }
  ...
  // Use same format as formatOrderListMessage
  ...
  uniqueOrders.forEach((order, index) => {
    ...
    // Build order block (SAME structure as formatOrderListMessage)
    const invoiceNumber = order.id || '-';
    message += `🧾 Invoice: ${invoiceNumber}\n`;
    message += `👤 Customer: ${customerName}\n`;
    message += `📞 Phone: ${phoneNumber}\n`;
    ...
    // PAYMENT INFO (NEW - added for /orders_unpaid)
    message += `💰 Payment:\n`;
    message += `• Total: Rp ${formatPrice(totalAmount)}\n`;
    message += `• Paid: Rp ${formatPrice(totalPaid)}\n`;
    message += `• Sisa: Rp ${formatPrice(remaining)}\n`;
...
export async function handleCancel(chatId, userId, orderId, sendMessage) {
  console.log(`🔍 [CANCEL] Command received - chatId: ${chatId}, userId: ${userId}, orderId: ${orderId || 'MISSING'}`);
...
/**
 * Handle /close command (renamed from /complete)
 ...
 * @param {Function} sendMessage - Function to send Telegram message
 */
export async function handleClose(chatId, userId, orderId, sendMessage) {
  console.log(`🔍 [CLOSE] Command received - chatId: ${chatId}, userId: ${userId}, orderId: ${orderId || 'MISSING'}`);
  ...
  if (!orderId) {
    await sendMessage(chatId, '❌ Format: `/close <ORDER_ID>`\n\nContoh: `/close DKM/20260104/000001`');
    return;
  }
  ...
  try {
    ...
    if (currentStatus.toLowerCase() !== 'delivered' && currentStatus.toLowerCase() !== 'confirmed') {
      console.warn(`⚠️ [CLOSE] Order ${orderId} status is ${currentStatus}, not DELIVERED/CONFIRMED`);
    ...
    // Note: sendMessage function is not available here, so we'll skip customer notification
    // Customer will see status update when they check order status
    console.log(`ℹ️ [CLOSE] Customer notification skipped (use order-status-notifications for full notification)`);
    ...
    console.error('❌ [CLOSE] Error completing order:', error);
...
// Keep handleComplete as alias for backward compatibility
export const handleComplete = handleClose;

/**
 * Handle /pay_confirm command
 * Confirm a pending payment verification (admin only)
 * @param {number} chatId - Telegram chat ID
 * @param {number} userId - Telegram user ID
 * @param {string} payment_id - Payment ID to confirm
 * @param {string} amountConfirmedInput - Optional: override amount_confirmed
 * @param {Function} sendMessage - Function to send Telegram message
 */
export async function handlePayConfirm(chatId, userId, payment_id, amountConfirmedInput, sendMessage) {
  console.log(`🔍 [PAY_CONFIRM] Command received - payment_id: ${payment_id || 'MISSING'}, amountConfirmedInput: ${amountConfirmedInput || 'use amount_input'}`);
  ...
  if (!payment_id) {
    await sendMessage(
      chatId,
      '❌ Format: `/pay_confirm <PAYMENT_ID> [amount]`\n\n' +
      '**Contoh:**\n' +
      '`/pay_confirm PAY/20260110/120530/1234` - Konfirmasi dengan amount_input\n' +
      '`/pay_confirm PAY/20260110/120530/1234 395000` - Konfirmasi dengan amount override'
    );
    return;
  }

  try {
    const { updatePaymentHistoryRecord, recalcOrderPaymentSummary } = await import('./google-sheets.js');
    const { parseIDRAmount, formatPrice } = await import('./price-calculator.js');
    
    // Read payment record to get current details
    const { getSheetHeaderMap } = await import('./google-sheets.js');
    const headerMap = await getSheetHeaderMap('Payment_History', { requireSnakeCase: false });
    
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.GOOGLE_SPREADSHEET_ID,
      range: `Payment_History!A:N`,
    });
    
    const rows = response.data.values || [];
    let paymentRow = null;
    for (let i = 1; i < rows.length; i++) {
      if (rows[i][0] === payment_id) {
        paymentRow = rows[i];
        break;
      }
    }
    ...
    if (!paymentRow) {
      await sendMessage(chatId, `❌ Payment ID \`${payment_id}\` tidak ditemukan di Payment_History.`);
      return;
    }
    ...
    const order_id = paymentRow[orderIdCol];
    const currentStatus = paymentRow[statusCol];
    const amount_input = parseFloat(paymentRow[amountInputCol]) || 0;
    
    // Check if already confirmed or rejected
    if (currentStatus === 'confirmed') {
      await sendMessage(chatId, `ℹ️ Payment \`${payment_id}\` sudah terkonfirmasi sebelumnya.`);
      return;
    }
    if (currentStatus === 'rejected') {
      await sendMessage(chatId, `ℹ️ Payment \`${payment_id}\` sudah ditolak sebelumnya. Tidak dapat dikonfirmasi.`);
      return;
    }
    
    // Determine amount_confirmed: use override if provided, otherwise use amount_input
    let amount_confirmed = amount_input;
    if (amountConfirmedInput) {
      const parsed = parseIDRAmount(amountConfirmedInput);
      if (parsed !== null) {
        amount_confirmed = parsed;
      }
    }
    
    // Update payment record: status = confirmed, amount_confirmed
    await updatePaymentHistoryRecord(payment_id, {
      status: 'confirmed',
    ...
    const summary = await recalcOrderPaymentSummary(order_id);
    
    console.log(`✅ [PAY_CONFIRM] Payment confirmed - payment_id: ${payment_id}, order_id: ${order_id}, amount_confirmed: ${amount_confirmed}`);
    ...
    await sendMessage(
      ...
      `💰 Payment ID: \`${payment_id}\`\n` +
      `📋 Order ID: \`${order_id}\`\n` +
      `💵 Jumlah Dikonfirmasi: Rp ${formatPrice(amount_confirmed)}\n\n` +
      `**Rekap Order:**\n` +
      `💳 Total Sudah Dibayar: Rp ${formatPrice(summary.total_paid)}\n` +
      `📊 Sisa: Rp ${formatPrice(summary.remaining_amount)}\n` +
      `✅ Status: ${summary.payment_status}`
    ...
    console.error('❌ [PAY_CONFIRM] Error confirming payment:', error);
    console.error('❌ [PAY_CONFIRM] Stack:', error.stack);
    await sendMessage(chatId, `❌ Terjadi kesalahan: ${error.message || 'Gagal mengkonfirmasi pembayaran.'}`);
  }
}

/**
 * Handle /pay_reject command
 * Reject a pending payment verification (admin only)
 * @param {number} chatId - Telegram chat ID
 * @param {number} userId - Telegram user ID
 * @param {string} payment_id - Payment ID to reject
 * @param {string} reason - Rejection reason
 * @param {Function} sendMessage - Function to send Telegram message
 */
export async function handlePayReject(chatId, userId, payment_id, reason, sendMessage) {
  console.log(`🔍 [PAY_REJECT] Command received - payment_id: ${payment_id || 'MISSING'}, reason: ${reason || 'no reason'}`);
  ...
  if (!payment_id) {
    await sendMessage(
      chatId,
      '❌ Format: `/pay_reject <PAYMENT_ID> [reason]`\n\n' +
      '**Contoh:**\n' +
      '`/pay_reject PAY/20260110/120530/1234` - Reject tanpa alasan\n' +
      '`/pay_reject PAY/20260110/120530/1234 Nominal tidak sesuai` - Reject dengan alasan'
    );
    return;
  }
  ...
  try {
    ...
    if (currentStatus === 'rejected') {
      await sendMessage(chatId, `ℹ️ Payment \`${payment_id}\` sudah ditolak sebelumnya.`);
      return;
    }
    
    // Update payment record: status = rejected
    await updatePaymentHistoryRecord(payment_id, {
      status: 'rejected',
      note: reason ? `Ditolak: ${reason}` : `Ditolak oleh admin (user ${userId})`,
    });
    
    // Recalculate order payment summary (will exclude this rejected payment)
    const summary = await recalcOrderPaymentSummary(order_id);
    
    console.log(`✅ [PAY_REJECT] Payment rejected - payment_id: ${payment_id}, order_id: ${order_id}, reason: ${reason || 'no reason'}`);
    ...
    console.error('❌ [PAY_REJECT] Error rejecting payment:', error);
    console.error('❌ [PAY_REJECT] Stack:', error.stack);
    await sendMessage(chatId, `❌ Terjadi kesalahan: ${error.message || 'Gagal menolak pembayaran.'}`);
  }
}
