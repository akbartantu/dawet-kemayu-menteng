...
// Payment_History sheet schema - AUDIT TRAIL FOR ALL PAYMENTS
// Column A: payment_id (format: PAY/YYYYMMDD/HHMMSS/XXXX)
// Column B: order_id
// Column C: payment_date
// Column D: payment_method (transfer/cash/qr/other)
// Column E: amount_input (angka yang diinput/terdeteksi)
// Column F: amount_confirmed (angka final yang disetujui; kosong sampai confirm)
// Column G: currency (default: IDR)
// Column H: status (pending_verification | confirmed | rejected)
// Column I: note (catatan admin: typo? DP? pelunasan?)
// Column J: proof_file_id (telegram file_id atau drive reference)
// Column K: proof_caption (caption yang dikirim user/admin)
// Column L: created_by (telegram user_id / username admin)
// Column M: created_at (timestamp)
// Column N: updated_at (timestamp)
...
export async function initializeStorage() {
  try {
    ...
    console.log(`🔍 [INIT] Initializing Payment_History sheet...`);
    try {
      await ensurePaymentHistorySheet();
      console.log(`✅ [INIT] Payment_History sheet ready`);
    } catch (error) {
 ...
 */
/**
 * Generate unique Order ID with year-based running number
 * Format: DKM/YYYYMMDD/NNNNNN
 * - YYYYMMDD: Order date (informational)
 * - NNNNNN: Running number (reset per YEAR, not per day)
 * 
 * Examples:
 * - First order 2026: DKM/20260108/000001
 * - Next day same year: DKM/20260109/000002
 * - Different day: DKM/20260215/000003
 * - First order 2027: DKM/20270103/000001 (reset)
 * 
...
export async function generateOrderId() {
  try {
    ...
    // Get the next order number for this YEAR (not for today)
    let orderNumber = 1;
    ...
    try {
      ...
      if (rows.length > 1) {
        // Find orders from this YEAR (filter by /YYYY pattern in Order ID)
        const yearPattern = `/${year}`;
        const yearOrders = rows.slice(1).filter(row => {
          const orderId = row[0] || '';
          // Check if Order ID contains /YYYY (e.g., /2026)
          return orderId.includes(yearPattern);
        });
        
        if (yearOrders.length > 0) {
          // Get the highest order number from this YEAR
          const orderNumbers = yearOrders.map(row => {
            const orderId = row[0] || '';
            // Extract last 6 digits (running number)
          ...
          console.log(`🔍 [GENERATE_ORDER_ID] Year: ${year}, Max number found: ${maxNumber}, Next: ${orderNumber}`);
        } else {
          console.log(`🔍 [GENERATE_ORDER_ID] Year: ${year}, No orders found for this year, starting from 1`);
        }
      } else {
        console.log(`🔍 [GENERATE_ORDER_ID] Orders sheet empty, starting from 1`);
      }
    } catch (error) {
      // Orders sheet doesn't exist yet, start from 1
      console.warn(`⚠️ [GENERATE_ORDER_ID] Error reading Orders sheet (will start from 1):`, error.message);
      orderNumber = 1;
    }
    ...
    const orderId = `DKM/${dateStr}/${orderNumberStr}`;
    console.log(`✅ [GENERATE_ORDER_ID] Generated Order ID: ${orderId}`);
    ...
    const fallbackId = `DKM/${new Date().toISOString().split('T')[0].replace(/-/g, '')}/${Date.now().toString().slice(-6)}`;
    console.warn(`⚠️ [GENERATE_ORDER_ID] Using fallback ID: ${fallbackId}`);
    return fallbackId;
  }
}
...
async function computeOrderTotals(orderData, priceList) {
  ...
  for (const note of packagingNotes) {
    ...
    if (noteLower.includes('plastic bag') || noteLower.includes('sisanya plastic')) {
      usePlasticBag = true;
      console.log(`🔍 [COMPUTE_TOTALS] Plastic bag requested`);
    }
...
export async function saveOrder(orderData, options = {}) {
  try {
    ...
    // NOTE: Duplicate check removed - saveOrder now always performs UPSERT (update if exists, insert if new)
    // The update logic below (line ~1428) handles existing orders correctly
    ...
    // Diagnostic log
    logger.debug(`[HEADER_MAP] delivery_method index=${headerMap.delivery_method !== undefined ? headerMap.delivery_method : -1}`);
    ...
    if (existingRowIndex) {
      ...
      if (itemsJsonColumnIndex !== undefined) {
        ...
        if (itemsJsonUpdate) {
          ...
          updateData.push({
            range: `Orders!${itemsJsonCol}${existingRowIndex}`,
            values: [[itemsJson]],
          });
    ...
    if (duplicateCheck && duplicateCheck !== existingRowIndex) {
      ...
      logger.error(`[SAVE_ORDER] CRITICAL: Duplicate detected for order ${orderId}! Existing at row ${existingRowIndex}, but found at row ${duplicateCheck}`);
...
export async function updateOrderPaymentWithEvidence(orderId, paymentAmount, evidenceFileId, evidenceType, telegramMessageId) {
  try {
    console.log(`🔍 [UPDATE_PAYMENT_EVIDENCE] Starting payment update with evidence - Order ID: ${orderId}, Amount: ${paymentAmount}`);
    ...
    if (order) {
      ...
      if (rowIndex) {
        ...
        if (notesColIndex !== undefined) {
          ...
          console.log(`✅ [UPDATE_PAYMENT_EVIDENCE] Evidence stored in notes_json`);
        }
      }
    }
...
export async function getOrderById(orderId) {
  try {
    ...
    if (order) {
      logger.debug(`✅ [GET_ORDER] Found order in Orders sheet`);
      return order;
    }
    
    // Fallback: Check WaitingList (secondary source)
    logger.debug(`[GET_ORDER] Not found in Orders, checking WaitingList...`);
    ...
    if (order) {
      logger.debug(`✅ [GET_ORDER] Found order in WaitingList sheet`);
      return order;
    }
    
    logger.debug(`⚠️ [GET_ORDER] Order ${normalizedInputId} not found in Orders or WaitingList`);
    return null;
  } catch (error) {
...
const PAYMENT_HISTORY_SHEET = 'Payment_History';

/**
 * Ensure Payment_History sheet exists with correct headers
 * @returns {Promise<void>}
 */
export async function ensurePaymentHistorySheet() {
  try {
    // Get spreadsheet to check if Payment_History sheet exists
    const spreadsheet = await sheets.spreadsheets.get({
      spreadsheetId: SPREADSHEET_ID,
    });

    const paymentHistorySheet = spreadsheet.data.sheets.find(
      s => s.properties.title === PAYMENT_HISTORY_SHEET
    );
    ...
    if (!paymentHistorySheet) {
      ...
      console.log('✅ Payment_History sheet created');
    }

    // Ensure headers exist
    await ensurePaymentHistoryHeaders();
  } catch (error) {
    console.error('❌ Error ensuring Payment_History sheet:', error.message);
    throw error;
  }
}

/**
 * Ensure Payment_History sheet has correct headers in row 1
 * @returns {Promise<void>}
 */
export async function ensurePaymentHistoryHeaders() {
  try {
    // Read first row to check if headers exist
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: `${PAYMENT_HISTORY_SHEET}!1:1`,
    });
    ...
    if (existingHeaders.length === 0) {
      ...
      await sheets.spreadsheets.values.update({
        ...
        valueInputOption: 'RAW',
        requestBody: {
          values: [PAYMENT_HISTORY_SCHEMA],
        },
      });
      console.log('✅ Payment_History sheet headers created');
      return;
    }

    // Check if all required columns exist
    const missingColumns = [];
    for (const col of PAYMENT_HISTORY_SCHEMA) {
    ...
    if (missingColumns.length > 0) {
      // Append missing columns to the right
      const startColumn = columnIndexToLetter(existingHeaders.length);
      const endColumn = columnIndexToLetter(existingHeaders.length + missingColumns.length - 1);
      const range = `${PAYMENT_HISTORY_SHEET}!${startColumn}1:${endColumn}1`;
      ...
      console.log(`✅ Added missing columns to Payment_History sheet: ${missingColumns.join(', ')}`);
    } else {
      console.log('✅ Payment_History sheet headers exist and are complete');
    }
  } catch (error) {
    console.error('❌ Error ensuring Payment_History headers:', error.message);
    throw error;
  }
}

/**
 * Generate unique payment_id: PAY/YYYYMMDD/HHMMSS/XXXX
...
/**
 * Append payment record to Payment_History sheet
 * @param {Object} paymentRecord - Payment record object
 * @param {string} paymentRecord.order_id - Order ID
 * @param {number} paymentRecord.amount_input - Amount entered/detected
 * @param {number|null} paymentRecord.amount_confirmed - Amount confirmed (null if pending)
 * @param {string} paymentRecord.payment_method - Payment method (transfer/cash/qr/other)
 * @param {string} paymentRecord.status - Status (pending_verification | confirmed | rejected)
 * @param {string} paymentRecord.note - Note/reason
 * @param {string} paymentRecord.proof_file_id - Telegram file_id or drive reference
 * @param {string} paymentRecord.proof_caption - Caption sent by user/admin
 * @param {string} paymentRecord.created_by - Telegram user_id or username
 * @returns {Promise<string>} Generated payment_id
 */
export async function appendPaymentHistory(paymentRecord) {
  try {
    console.log(`💰 [PAYMENT_HISTORY] Appending payment record for order: ${paymentRecord.order_id}`);
    
    // Ensure Payment_History sheet exists
    await ensurePaymentHistorySheet();
    ...
    // Build row data matching PAYMENT_HISTORY_SCHEMA order
    const rowData = [
      payment_id,
      paymentRecord.order_id || '',
      payment_date,
      paymentRecord.payment_method || 'transfer',
      paymentRecord.amount_input || 0,
      paymentRecord.amount_confirmed || '', // Empty if pending
      paymentRecord.currency || 'IDR',
      paymentRecord.status || 'pending_verification',
      paymentRecord.note || '',
      paymentRecord.proof_file_id || '',
      paymentRecord.proof_caption || '',
      paymentRecord.created_by || '',
      created_at,
      updated_at,
    ];
    ...
    await retryWithBackoff(async () => {
      return await sheets.spreadsheets.values.append({
        spreadsheetId: SPREADSHEET_ID,
        range: `${PAYMENT_HISTORY_SHEET}!A:N`,
        valueInputOption: 'RAW',
        requestBody: {
          values: [rowData],
        },
    ...
    console.log(`✅ [PAYMENT_HISTORY] Payment record created: ${payment_id}`);
    return payment_id;
  } catch (error) {
    console.error(`❌ [PAYMENT_HISTORY] Error appending payment:`, error.message);
    throw error;
  }
}

/**
 * Update payment record status in Payment_History sheet
 * @param {string} payment_id - Payment ID to update
 * @param {Object} updates - Fields to update
 * @param {string} updates.status - New status
 * @param {number} updates.amount_confirmed - Confirmed amount
 * @param {string} updates.note - Note/reason
 * @returns {Promise<void>}
 */
export async function updatePaymentHistoryRecord(payment_id, updates) {
  try {
    console.log(`💰 [PAYMENT_HISTORY] Updating payment record: ${payment_id}`);
    
    // Get header map
    const headerMap = await getSheetHeaderMap(PAYMENT_HISTORY_SHEET, { requireSnakeCase: false });
    
    // Find row by payment_id
    const response = await retryWithBackoff(async () => {
      return await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: `${PAYMENT_HISTORY_SHEET}!A:A`,
      });
    });
    
    const paymentIds = response.data.values || [];
    let rowIndex = -1;
    for (let i = 1; i < paymentIds.length; i++) {
      if (paymentIds[i][0] === payment_id) {
    ...
    if (rowIndex === -1) {
      throw new Error(`Payment record ${payment_id} not found in Payment_History`);
    }
    ...
    const updateColumn = (internalKey, value) => {
      ...
      if (colIndex !== undefined) {
        ...
        updateData.push({
          range: `${PAYMENT_HISTORY_SHEET}!${col}${rowIndex}`,
          values: [[value]],
        });
        console.log(`🔍 [PAYMENT_HISTORY] Updating column "${internalKey}" (${col}${rowIndex}) = ${value}`);
        return true;
      }
      console.warn(`⚠️ [PAYMENT_HISTORY] Column "${internalKey}" not found in header map, skipping update`);
      return false;
    };
    
    // Update fields
    if (updates.status !== undefined) {
      updateColumn('status', updates.status);
    }
    if (updates.amount_confirmed !== undefined) {
      updateColumn('amount_confirmed', updates.amount_confirmed);
    }
    if (updates.note !== undefined) {
      updateColumn('note', updates.note);
    }
    updateColumn('updated_at', updated_at);
    ...
    if (updateData.length > 0) {
      ...
      console.log(`✅ [PAYMENT_HISTORY] Payment record updated: ${payment_id}`);
    }
  } catch (error) {
    console.error(`❌ [PAYMENT_HISTORY] Error updating payment:`, error.message);
    throw error;
  }
}

/**
 * Recalculate order payment summary from Payment_History and update Orders sheet
 * @param {string} order_id - Order ID to recalculate
 * @returns {Promise<Object>} Payment summary { total_paid, total_paid_pending, remaining_amount, payment_status, last_payment_at, last_payment_method }
 */
export async function recalcOrderPaymentSummary(order_id) {
  try {
    console.log(`🔄 [PAYMENT_SUMMARY] Recalculating payment summary for order: ${order_id}`);
    
    // Get order to get total_amount
    ...
    const totalAmount = order.total_amount || order.final_total || 0;
    console.log(`🔍 [PAYMENT_SUMMARY] Order total_amount: ${totalAmount}`);
    
    // Get all payment records for this order from Payment_History
    await ensurePaymentHistorySheet();
    const headerMap = await getSheetHeaderMap(PAYMENT_HISTORY_SHEET, { requireSnakeCase: false });
    ...
    const rows = response.data.values || [];
    if (rows.length <= 1) {
      // No payment records, return zeros
      console.log(`ℹ️ [PAYMENT_SUMMARY] No payment records found for order: ${order_id}`);
      return {
        total_paid: 0,
    ...
    // Get column indices
    const orderIdCol = headerMap.order_id ?? 1;
    const statusCol = headerMap.status ?? 7;
    const amountInputCol = headerMap.amount_input ?? 4;
    const amountConfirmedCol = headerMap.amount_confirmed ?? 5;
    const paymentMethodCol = headerMap.payment_method ?? 3;
    const updatedAtCol = headerMap.updated_at ?? 13;
    
    // Filter and sum payments for this order
    let total_paid = 0;
    let total_paid_pending = 0;
    let last_payment_at = null;
    let last_payment_method = null;
    
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      if (!row || row.length === 0) continue;
      
      const rowOrderId = row[orderIdCol];
      if (rowOrderId !== order_id) continue;
      
      const status = row[statusCol] || '';
      const amount_input = parseFloat(row[amountInputCol]) || 0;
      const amount_confirmed = parseFloat(row[amountConfirmedCol]) || 0;
      const payment_method = row[paymentMethodCol] || '';
      const updated_at = row[updatedAtCol] || '';
      
      if (status === 'confirmed' && amount_confirmed > 0) {
        total_paid += amount_confirmed;
        // Track last confirmed payment
        if (!last_payment_at || updated_at > last_payment_at) {
          last_payment_at = updated_at;
          last_payment_method = payment_method;
        }
      } else if (status === 'pending_verification' && amount_input > 0) {
        total_paid_pending += amount_input;
      }
      // Skip rejected payments
    }
    
    console.log(`🔍 [PAYMENT_SUMMARY] Total paid (confirmed): ${total_paid}, Pending: ${total_paid_pending}`);
    
    // Calculate remaining and status
    const remaining_amount = Math.max(0, totalAmount - total_paid);
    let payment_status = 'UNPAID';
    if (total_paid >= totalAmount) {
      payment_status = 'PAID';
    } else if (total_paid > 0) {
      payment_status = 'PARTIAL';
    }
    
    // If there are pending payments, add "PENDING" flag
    if (total_paid_pending > 0) {
      payment_status = `${payment_status} (${total_paid_pending} pending)`;
    }
    ...
    console.log(`✅ [PAYMENT_SUMMARY] Summary:`, summary);
    
    // Update Orders sheet with summary
    await updateOrderPaymentFields(order_id, summary);
    
    return summary;
  } catch (error) {
    console.error(`❌ [PAYMENT_SUMMARY] Error recalculating payment summary:`, error.message);
    throw error;
  }
}

/**
 * Update Orders sheet with payment summary fields
 * Ensures required payment columns exist before updating
 * @param {string} order_id - Order ID
 * @param {Object} summary - Payment summary object
 * @returns {Promise<void>}
 */
export async function updateOrderPaymentFields(order_id, summary) {
  try {
    console.log(`🔄 [UPDATE_ORDER_PAYMENT] Updating Orders sheet for order: ${order_id}`);
    
    // Ensure payment summary columns exist in Orders sheet
    await ensureOrdersPaymentSummaryColumns();
    
    // Find row index
    const rowIndex = await findRowByOrderId('Orders', order_id);
    if (!rowIndex) {
      throw new Error(`Order ${order_id} not found in Orders sheet`);
    }
    ...
    const updateColumn = (internalKey, value) => {
      ...
      if (colIndex !== undefined) {
        ...
        console.log(`🔍 [UPDATE_ORDER_PAYMENT] Updating column "${internalKey}" (${col}${rowIndex}) = ${value}`);
        return true;
      }
      console.warn(`⚠️ [UPDATE_ORDER_PAYMENT] Column "${internalKey}" not found in header map, skipping update`);
      return false;
    };
    
    // Update payment summary fields
    updateColumn('total_paid', summary.total_paid || 0);
    updateColumn('paid_amount', summary.total_paid || 0); // Alias for total_paid (existing column)
    updateColumn('total_paid_pending', summary.total_paid_pending || 0);
    updateColumn('remaining_balance', summary.remaining_amount || 0); // Use existing column name
    updateColumn('payment_status', summary.payment_status || 'UNPAID');
    if (summary.last_payment_at) {
      updateColumn('last_payment_at', summary.last_payment_at);
    }
    if (summary.last_payment_method) {
      updateColumn('last_payment_method', summary.last_payment_method);
    }
    updateColumn('updated_at', updated_at);
    ...
    if (updateData.length > 0) {
      ...
      console.log(`✅ [UPDATE_ORDER_PAYMENT] Orders sheet updated for: ${order_id}`);
    }
  } catch (error) {
    console.error(`❌ [UPDATE_ORDER_PAYMENT] Error updating Orders sheet:`, error.message);
    throw error;
  }
}

/**
 * Ensure Orders sheet has payment summary columns
 * Adds: total_paid, total_paid_pending, last_payment_at, last_payment_method
 * @returns {Promise<void>}
 */
export async function ensureOrdersPaymentSummaryColumns() {
  try {
    // Read Orders sheet headers
    const response = await retryWithBackoff(async () => {
      return await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID,
        range: `Orders!1:1`,
    ...
    // Required payment summary columns (in addition to existing paid_amount, remaining_balance, payment_status)
    ...
    if (missingColumns.length > 0) {
      ...
      const startColumn = columnIndexToLetter(existingHeaders.length);
      const endColumn = columnIndexToLetter(existingHeaders.length + missingColumns.length - 1);
      const range = `Orders!${startColumn}1:${endColumn}1`;
      ...
      console.log(`✅ [ORDERS_PAYMENT_COLS] Added missing columns to Orders sheet: ${missingColumns.join(', ')}`);
      
      // Invalidate header cache
      invalidateHeaderCache('Orders');
    } else {
      console.log('✅ [ORDERS_PAYMENT_COLS] Orders sheet payment columns exist');
    }
  } catch (error) {
    console.error('❌ [ORDERS_PAYMENT_COLS] Error ensuring Orders payment columns:', error.message);
    throw error;
  }
}
...
/**
 * Delete obsolete WaitingList sheet (safe one-time cleanup)
 * This function safely removes the WaitingList sheet if it exists.
 * All orders are now tracked in Orders sheet (status='waiting' for future orders).
 * Reminders are tracked in Reminders sheet.
 */
export async function deleteWaitingListSheet() {
  try {
    console.log('🔄 [DELETE_WAITINGLIST] Checking for obsolete WaitingList sheet...');
    
    const spreadsheet = await sheets.spreadsheets.get({
      spreadsheetId: SPREADSHEET_ID,
    });
    
    const waitingListSheet = spreadsheet.data.sheets.find(
      s => s.properties.title === 'WaitingList'
    );
    
    if (!waitingListSheet) {
      console.log('ℹ️ [DELETE_WAITINGLIST] WaitingList sheet not found (already deleted or never existed)');
      return { success: true, message: 'WaitingList sheet not found' };
    }
    
    console.log('🗑️ [DELETE_WAITINGLIST] Found WaitingList sheet, deleting...');
    ...
    await sheets.spreadsheets.batchUpdate({
      ...
      requestBody: {
        requests: [{
          deleteSheet: {
            sheetId: waitingListSheet.properties.sheetId
          }
        }]
      }
    });
    
    console.log('✅ [DELETE_WAITINGLIST] Successfully deleted obsolete WaitingList sheet');
    return { success: true, message: 'WaitingList sheet deleted' };
  } catch (error) {
    console.error('❌ [DELETE_WAITINGLIST] Error deleting WaitingList sheet:', error.message);
    // Non-critical error - don't fail server startup
