...
export async function getSheetHeaderMap(sheetName, options = {}) {
  try {
    ...
    .length} internal keys to columns`);
...
export async function ensureOrdersPaymentHeaders() {
  try {
    ...
    if (missingHeaders.length > 0) {
      ...
      to Orders sheet: ${missingHeaders.join(', ')}`);
    }
  } catch (error) {
    :', error.message);
...
export async function ensureWaitingListPaymentHeaders() {
  try {
    ...
    if (missingHeaders.length > 0) {
      ...
      to WaitingList sheet: ${missingHeaders.join(', ')}`);
    }
  } catch (error) {
    :', error.message);
    // Non-critical, continue
    ...
    const orders = rows.slice(1, limit + 1).map(row => {
      ...
      return {
        ...
        total_amount: (() => {
          ...
          if (parsed > 0) {
            for order ${getValue('order_id', 'unknown')}`);
          }
          return parsed;
        })(),
...
export async function reportLegacyTitleCaseColumns(sheetName) {
  try {
    ...
    .`);
    .join(', ')} in Google Sheets and delete.`);
    return {
      sheetName,
      legacyColumns,
